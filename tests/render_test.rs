...
fn render_pr_comment_includes_gate_and_stages() {
    ...
    let data: BenchmarkData =
        serde_json::from_str(&fs::read_to_string(root.join("tests/fixtures/benchmark-data.json")).unwrap())
            .unwrap();
    let gate: GateResult =
        serde_json::from_str(&fs::read_to_string(root.join("tests/fixtures/gate-result.json")).unwrap())
            .unwrap();
