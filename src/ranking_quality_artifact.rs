//! Ranking-quality benchmark artifact helpers.
...
pub const RANKING_QUALITY_JSON_NAME: &str = "ranking-quality-benchmark-data.json";

pub struct ExtractedRankingQualityArtifact {
    _temp_dir: tempfile::TempDir,
    pub json_path: PathBuf,
}

pub fn extract_ranking_quality_artifact_zip(
    bytes: &[u8],
) -> Result<ExtractedRankingQualityArtifact> {
    ...
    for index in 0..archive.len() {
        ...
        let file_name = safe_name
            ...
            .unwrap_or("");
        if file_name != RANKING_QUALITY_JSON_NAME {
            continue;
        }
    ...
    let json_path =
        json_path.ok_or_else(|| bot_err(format!("{RANKING_QUALITY_JSON_NAME} not in artifact")))?;
    Ok(ExtractedRankingQualityArtifact {
        _temp_dir: dir,
        json_path,
    })
}
