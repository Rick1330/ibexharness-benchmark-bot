...
pub fn validate_badge_svg(bytes: &[u8]) -> Result<()> {
    ...
    let normalized = normalize_svg_for_scan(text);
    let forbidden = [
        "<script",
        "onload=",
        "onerror=",
...
fn normalize_svg_for_scan(text: &str) -> String {
...
mod tests {
    ...
    #[test]
    fn rejects_obfuscated_event_handler() {
        assert!(validate_badge_svg(br#"<svg on load =alert(1)></svg>"#).is_err());
    }
