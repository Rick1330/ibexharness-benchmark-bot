//! Publish extraction-quality benchmark history to the harness repo.

use std::fs;

use crate::config::{DATA_PR_BRANCH, EXTRACTION_QUALITY_BENCHMARK_DATA_PATH};
use crate::error::{bot_err, Result};
use crate::extraction_quality_artifact::extract_extraction_quality_artifact_zip;
use crate::extraction_quality_validate::{
    cross_check_extraction_quality_artifact_run, extraction_quality_max_published_run_number,
...
use crate::publish_shared::{
    extraction_quality_head_already_on_branch, upsert_combined_data_pr, UpsertDataPr,
};
use crate::verify;
...
pub async fn publish_extraction_quality_benchmark_data(
    client: &GitHubClient,
    repo_full: &str,
    ...
    if let Some(existing) = client.find_open_pr(repo_ref, DATA_PR_BRANCH).await? {
        if extraction_quality_head_already_on_branch(client, repo_ref, head_sha).await? {
    ...
    let zip = client
        .download_extraction_quality_artifact_zip(repo_ref, payload.run_id)
        .await?;
    let extracted = extract_extraction_quality_artifact_zip(&zip)?;
    validate_extraction_quality_file(&extracted.json_path)?;

    let json_bytes = fs::read(&extracted.json_path)
        .map_err(|err| bot_err(format!("read extraction-quality benchmark json: {err}")))?;
    let benchmark_data: ExtractionQualityBenchmarkData = serde_json::from_slice(&json_bytes)
        .map_err(|err| bot_err(format!("decode extraction-quality benchmark json: {err}")))?;
    cross_check_extraction_quality_artifact_run(
        &benchmark_data,
        &run,
        payload.run_id,
        payload.run_number,
    ...
    let pr = upsert_combined_data_pr(UpsertDataPr {
        ...
        extraction_run_url: run.html_url.as_deref(),
        extraction_run_number: Some(payload.run_number),
    })
    .await?;
...
async fn ensure_not_replay(
    ...
    let data: ExtractionQualityBenchmarkData = serde_json::from_slice(&bytes)
        .map_err(|err| bot_err(format!("decode published extraction-quality data: {err}")))?;
    if extraction_quality_published_sha_exists(&data, head_sha) {
        return Err(bot_err("head_sha already published on main".to_string()));
    }
    if let Some(max_run) = extraction_quality_max_published_run_number(&data) {
