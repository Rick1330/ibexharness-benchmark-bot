//! Write-pipeline benchmark artifact helpers.
...
pub const WRITE_PIPELINE_JSON_NAME: &str = "write-pipeline-benchmark-data.json";

pub struct ExtractedWritePipelineArtifact {
    _temp_dir: tempfile::TempDir,
    pub json_path: PathBuf,
}

pub fn extract_write_pipeline_artifact_zip(bytes: &[u8]) -> Result<ExtractedWritePipelineArtifact> {
    ...
    for index in 0..archive.len() {
        ...
        let file_name = safe_name
            ...
            .unwrap_or("");
        if file_name != WRITE_PIPELINE_JSON_NAME {
            continue;
        }
    ...
    let json_path =
        json_path.ok_or_else(|| bot_err(format!("{WRITE_PIPELINE_JSON_NAME} not in artifact")))?;
    Ok(ExtractedWritePipelineArtifact {
        _temp_dir: dir,
        json_path,
    })
}
