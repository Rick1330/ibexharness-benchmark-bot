//! Extraction-quality benchmark artifact helpers.
...
pub const EXTRACTION_QUALITY_JSON_NAME: &str = "extraction-quality-benchmark-data.json";

pub struct ExtractedExtractionQualityArtifact {
    _temp_dir: tempfile::TempDir,
    pub json_path: PathBuf,
}

pub fn extract_extraction_quality_artifact_zip(
    bytes: &[u8],
) -> Result<ExtractedExtractionQualityArtifact> {
    ...
    for index in 0..archive.len() {
        ...
        let file_name = safe_name
            ...
            .unwrap_or("");
        if file_name != EXTRACTION_QUALITY_JSON_NAME {
            continue;
        }
    ...
    let json_path = json_path
        .ok_or_else(|| bot_err(format!("{EXTRACTION_QUALITY_JSON_NAME} not in artifact")))?;
    Ok(ExtractedExtractionQualityArtifact {
        _temp_dir: dir,
        json_path,
    })
}
